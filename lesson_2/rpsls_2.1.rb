# RPSLS 2.1
# This change merges the Display class with the RPSGame class. Since only the 
# RPSGame class uses the functionality in the Display class, it's an artificial
# break.
...
class RPSGame
  ...
  def input_game_count
    loop do
      ...
      prompt "Sorry, I need an integer between 1 and 20. Let's try again."
    end
  end
  ...
  def play_end
    show_final_winner(computer.wins == games ? computer : human)
    puts
    if play_again?
      wait 0.5
      play(false)
  ...
  def play_start(first_game)
    if first_game then show_welcome_message
    else reset_game
    end
    wait 0.5
  ...
  # Display methods. This is all the methods responsible for showing 
  # anything on the screen. Exceptions are methods that get input from
  # the user.
